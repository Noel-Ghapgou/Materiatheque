...
function(promiseTracker, $compile, $templateCache, cgBusyTemplateName, $http, $animate)
{
    return {
        ...
        link : function(scope, element, attrs, fn)
        {
            ...
            $http.get(indicatorTemplateName, {
                cache : $templateCache
            }).success(function(indicatorTemplate)
            {
                ...
                options.animate = typeof options.animate === 'undefined' ? true : options.animate;
                var animation = options.animate ? 'cg-busy-animation' : '';

                var busyClass = angular.isDefined(options.busyClass) ? options.busyClass : 'cg-busy';

                var template = '<div class="' + busyClass + ' ' + animation + ' ng-hide" ng-show="isActive()">' + backdrop + indicatorTemplate + '</div>';
                var templateElement = $compile(template)(scope);

                var elm = angular.element(templateElement.children()[options.backdrop ? 1 : 0]);
                if (!options.relative) {
                    elm.css('position', 'absolute').css('top', 0).css('left', 0).css('right', 0).css('bottom', 0);
                }
...
angular.module("cgBusy").run(["$templateCache",
